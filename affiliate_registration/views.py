...
def registration_form(request):
    if request.method == "POST":
        ...
        if registration_form.is_valid():
            ...
            new_reputation = Reputation.objects.create(affiliate_id=new_affiliate.id)
            return render(request, "success_message.html")
        else:
            return render(
                request, "registration_form.html", {"form": registration_form}
            )
    else:
        registration_form = forms.Register_Form()
        return render(request, "registration_form.html", {"form": registration_form})
